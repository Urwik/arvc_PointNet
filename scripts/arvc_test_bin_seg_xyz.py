...
def compute_best_threshold(pred_, gt_):
    trshld_per_batch = []
    for batch in range(len(pred_)):
        fpr, tpr, thresholds = metrics.roc_curve(gt_[batch], pred_[batch])
        gmeans = np.sqrt(tpr * (1 - fpr))
        ix = np.argmax(gmeans)
        trshld_per_batch.append(thresholds[ix])

    return sum(trshld_per_batch)/len(trshld_per_batch)
...
def save_pred_as_ply(data_, pred_fix_, output_dir_, filename_):
    ...
    for i in range(batch_size):
        ...
        cloud = np.hstack((actual_data, actual_pred))
        filename = filename_[0]
        np2ply(cloud, out_dir, filename, features=feat_xyzi, binary=True)
        saved_clouds += 1
...
if __name__ == '__main__':
    ...
    BATCH_SIZE = 1
    MAX_K = 1
    THRESHOLD = 0.21777047057871127

    # PATHS
    DATASET_DIR = os.path.abspath('/media/arvc/data/datasets/ARVC_GZF/test/ply_binary_class')
    BEST_MODEL_PATH = os.path.abspath('//model_save/13.01.2023.09 - 16_50_0.001/best_model.pth')
    OUT_CLOUDS_DIR = os.path.abspath('//pred_clouds')
    saved_clouds = 0
    ...
    # INSTANCE DATASET
    dataset = PLYDataset(data_root=DATASET_DIR,
                         _features=(0, 1, 2),
                         transform=None)
    ...
    avg_f1_score = np.mean(np.array(results[1]))
    avg_precision = np.mean(np.array(results[2]))
    avg_recall = np.mean(np.array(results[3]))

    confusion_matrix_list = np.array(results[4])
    avg_conf_matrix = np.mean(confusion_matrix_list, axis=0)
    print(f'Avg F1_score: {avg_f1_score}')
    print(f'Avg Precision: {avg_precision}')
    print(f'Avg Recall: {avg_recall}')
    print(f'TN: {avg_conf_matrix[0]}, FP: {avg_conf_matrix[1]}, FN: {avg_conf_matrix[2]}, TP: {avg_conf_matrix[3]}')
    print("Done!")
