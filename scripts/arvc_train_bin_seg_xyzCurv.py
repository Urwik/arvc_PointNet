...
def val_in_train(device_, dataloader_, model_, loss_fn_):
    ...
    with torch.no_grad():
        for batch, (data, label, _) in enumerate(dataloader_):
            ...
            # f1_score = compute_f1_metric(label, pred)
            # f1_lst.append(f1_score)

            avg_loss = loss_fn_(pred, label)
            loss_lst.append(avg_loss.item())

        total_avg_loss = np.mean(loss_lst)
        # total_avg_f1 = np.mean(f1_lst)

    # return total_avg_loss, total_avg_f1
    return total_avg_loss, 0


def compute_f1_metric(label_, pred_):
...
if __name__ == '__main__':
    ...
    if machine_name == 'arvc-Desktop':
        ...
        OUTPUT_PATH = os.path.abspath('/home/arvc/PycharmProjects/arvc_PointNet/model_save/')
    else:
        ROOT_DIR = os.path.abspath('/home/arvc/Fran/data/datasets/ARVC_GZF/train/ply_xyzlabelnormal')
        OUTPUT_PATH = os.path.abspath('/home/arvc/Fran/PycharmProjects/arvc_PointNet/model_save/bin_seg/')
