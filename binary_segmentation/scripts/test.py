...
def get_representative_clouds(f1_score_, precision_, recall_, files_list_):
    ...
    print(f'Min precision cloud: {str(files_list_[min_pre_idx])}')
    print(f'Max recall cloud: {str(files_list_[max_rec_idx])}')
    print(f'Min recall cloud: {str(files_list_[min_rec_idx])}')
...
def get_exported_results():
    ...
    exported_results = list(map(str, exported_results))
